...
- **`main`** — The source of truth for all XDR definitions. All changes should
  be made to this branch. Definitions may contain `#ifdef`, `#else`, `#endif`
  preprocessor directives to gate features.
- **`curr`** — Auto-generated from `main` with no features enabled. Represents
  the current protocol XDR.
- **`next`** — Auto-generated from `main` with all features enabled. Represents
  the next protocol XDR, including in-development changes.

The `curr` and `next` branches are generated by a GitHub Actions workflow that
runs on every push to `main`. The workflow uses `stellar-xdr xfile preprocess`
to evaluate the preprocessor directives. Do not commit directly to `curr` or
`next` — all changes should be made on `main`.
...
All changes are made on the `main` branch. Changes that should only appear in
`next` or custom builds (not `curr`) must be wrapped in preprocessor directives.
Use a narrow feature flag named after the feature:

```
#ifdef CAP73_SAC_CREATE_ACCOUNTS
struct MyNewType
{
    int32 value;
};
